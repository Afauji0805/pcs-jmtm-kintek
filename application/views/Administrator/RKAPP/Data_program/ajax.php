...
$(document).ready(function() {
    ...
    function parseToDateObj(dateStr) {
        ...
        if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
            const p = dateStr.split('-'); return new Date(p[0], p[1]-1, p[2]);
        }
        let m;
        if (m = dateStr.match(/^(\d{2})[\/-](\d{2})[\/-](\d{4})$/)) {
...
/* ----------------------------------- DETAIL ----------------------------------- */
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status === 'success') {
                    ...
                    $('#detail_tanggal_program').text(formatDateForDisplay(d.tanggal_program));
                    ...
                    $('#detail_tanggal_mulai_kontrak').text(formatDateForDisplay(d.tanggal_mulai_kontrak));
                    $('#detail_tanggal_selesai_kontrak').text(formatDateForDisplay(d.tanggal_selesai_kontrak));
                    $('#detail_durasi_kontrak').text((d.durasi_kontrak || 0) + ' Hari');
                    $('#detail_tanggal_mulai_pho').text(formatDateForDisplay(d.tanggal_mulai_pho));
                    $('#detail_tanggal_selesai_pho').text(formatDateForDisplay(d.tanggal_selesai_pho));
                    ...
                    $('#staticBackdrop-detail-program .btn-warning').data('id', d.id_program ||'');
    ...
    $(document).on('click', '#staticBackdrop-detail-program .btn-warning', function() {
        ...
        const data = {
            ...
            tanggal_selesai_kontrak: $('#detail_tanggal_selesai_kontrak').text().trim(), // dd/mm/yyyy
        ...
        setTimeout(function() {
            ...
            $('#ubah_tanggal_program').val( convertToDateInput(data.tanggal_program) ); // yyyy-mm-dd
            ...
            $('#ubah_tanggal_mulai_kontrak').val( convertToDateInput(data.tanggal_mulai_kontrak) );
            $('#ubah_durasi_kontrak').val( data.durasi_kontrak || '' );
            $('#ubah_tanggal_selesai_kontrak').val( (data.tanggal_selesai_kontrak || '').replace(/\//g,'-') );
    ...
    $('#btn-update').on('click', function(e) {
        ...
        // CSRF token (jika perlu)
        const csrfName = $('#csrf_token').attr('name');
        const csrfHash = $('#csrf_token').val();
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status === 'success') {
                    ...
                    .then(() => {
                        ...
                        if ($.fn.DataTable.isDataTable('#table_program')) {
                            $('#table_program').DataTable().ajax.reload(null, false);
                        }
                    });
                } else {
                    Swal.fire({ icon: 'error', title: 'Gagal', text: res.message || 'Gagal menyimpan.'});
                }
            ...
            error: function(xhr) {
                ...
                Swal.fire({ icon: 'error', title: 'Error', text: xhr.statusText });
            }
        });
    });
